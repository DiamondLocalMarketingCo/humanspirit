...
export default function IntroLoader() {
  return (
    ...
    >
      <div className="flex w-full max-w-[520px] flex-col items-center gap-4 px-6 sm:max-w-[560px]">
        <motion.span
          className="text-sm uppercase tracking-[0.85em] text-white/80"
        ...
        <div className="relative h-2 w-full overflow-hidden rounded-full bg-white/10">
          ...
          <div className="absolute inset-y-0 left-0 right-0 grid grid-cols-12 gap-2 px-1">
            {Array.from({ length: 10 }).map((_, index) => (
              <span
                key={index}
                className="block h-px w-full rounded-full bg-white/15"
              />
            ))}
          </div>
        </div>
        ...
        <motion.span
          className="text-[0.65rem] uppercase tracking-[0.75em] text-white/40"
